...
await mkdir("tmp/ui-quality/worldmap-open-node-tooltips", { recursive: true });
...
try {
  ...
  for (const viewport of viewports) {
    ...
    for (const auditCase of auditCases) {
      ...
      const initial = await readWorldMapOpenNodeTooltipAudit(page, auditCase);
      if (initial.tooltip.visible) {
        throw new Error(`${auditCase.key}/${viewport.key}: tooltip visible before open-node hover`);
      }
      ...
      const hoverAudit = await readWorldMapOpenNodeTooltipAudit(page, auditCase);
      assertWorldMapOpenNodeTooltip(`${auditCase.key}/${viewport.key}`, hoverAudit, auditCase, viewport, seeded);

      const screenshot = path.join("tmp", "ui-quality", "worldmap-open-node-tooltips", `${auditCase.key}-open-node-tooltip-v1-${viewport.suffix}.png`);
      ...
      results.push({
        ...
        visibleHaloImages: hoverAudit.visibleHaloImages,
        ...
        targetCompletedFamily: hoverAudit.targetCompletedFamily,
        targetCompletedBody: hoverAudit.targetCompletedBodyAtTarget ? "ok" : "missing",
        targetCompletedFrame: hoverAudit.targetCompletedFrameAtTarget ? "ok" : "missing",
        targetCompletedBadge: hoverAudit.targetCompletedBadgeAtTarget ? "ok" : "missing",
...
async function startServer() {
  ...
  throw new Error("No free WorldMap open-node tooltip audit port found");
}
...
async function seedWorldMapState(page, baseUrl, auditCase) {
  ...
  const seeded = await page.evaluate(({ completedCount, currentStageIndex, targetIndex }) => {
    ...
    context.save.currentRun = {
      ...
      log: [...new Set([...(context.save.currentRun.log ?? []), "audit:worldmap_open_node_tooltip"])]
    };
  ...
  if (!seeded.ok) {
    throw new Error(`WorldMap open-node tooltip seed failed: ${JSON.stringify(seeded)}`);
  }
...
async function readWorldMapOpenNodeTooltipAudit(page, auditCase) {
  return page.evaluate(({ auditCase, stageNodes }) => {
    ...
    return {
      ...
      hoverHaloAtTargetStage: Boolean(targetNode) && haloImages.some((image) => (
...
function assertWorldMapOpenNodeTooltip(label, audit, auditCase, viewport, seeded) {
  ...
  if (!audit.targetCompletedBadgeAtTarget || !audit.targetCompletedBadgeStyleAtTarget) {
    throw new Error(`${label}: target completed badge missing or incorrectly styled during hover ${JSON.stringify({
  ...
  if (!audit.targetCompletedBodyAtTarget || !audit.targetCompletedBodyStyleAtTarget) {
    throw new Error(`${label}: target completed body missing or incorrectly styled during hover ${JSON.stringify({
      targetCompletedFamily: audit.targetCompletedFamily,
  ...
  if (!audit.targetCompletedFrameAtTarget || !audit.targetCompletedFrameStyleAtTarget) {
    throw new Error(`${label}: target completed frame missing or incorrectly styled during hover ${JSON.stringify({
      targetCompletedFamily: audit.targetCompletedFamily,
  ...
  if (!audit.targetHasNoCurrentStack || !audit.targetHasNoLockedStack) {
    throw new Error(`${label}: target has conflicting state overlays during hover ${JSON.stringify({
      targetCompletedFamily: audit.targetCompletedFamily,
  ...
  if (!audit.markerAtCurrentStage || audit.visibleCurrentMarkerImages !== 1) {
    throw new Error(`${label}: current marker changed during hover ${JSON.stringify({
      visibleCurrentMarkerImages: audit.visibleCurrentMarkerImages,
      markerAtCurrentStage: audit.markerAtCurrentStage
    })}`);
  }
  if (audit.visibleHaloImages !== 2 || !audit.currentHaloAtCurrentStage || !audit.hoverHaloAtTargetStage) {
    throw new Error(`${label}: expected current halo plus target hover halo, got ${JSON.stringify({
      visibleHaloImages: audit.visibleHaloImages,
      currentHaloAtCurrentStage: audit.currentHaloAtCurrentStage,
      hoverHaloAtTargetStage: audit.hoverHaloAtTargetStage
    })}`);
  }
  if (!audit.bodyAtCurrentStage || audit.visibleCurrentBodyImages !== 1) {
    throw new Error(`${label}: current body changed during hover ${JSON.stringify({
  ...
  if (!audit.frameAtCurrentStage || audit.visibleCurrentFrameImages !== 1) {
    throw new Error(`${label}: current frame changed during hover ${JSON.stringify({
  ...
  if (!audit.statusAtCurrentStage || audit.visibleCurrentStatusImages !== 1) {
    throw new Error(`${label}: current status badge changed during hover ${JSON.stringify({
  ...
  if (audit.visibleCurrentBaseBodyImages !== (audit.expectedLateCurrent ? 0 : 1)) {
    throw new Error(`${label}: wrong base current body count during hover ${audit.visibleCurrentBaseBodyImages}`);
  }
  if (audit.visibleCurrentLateBodyImages !== (audit.expectedLateCurrent ? 1 : 0)) {
    throw new Error(`${label}: wrong late current body count during hover ${audit.visibleCurrentLateBodyImages}`);
  }
  if (audit.visibleCurrentBaseFrameImages !== (audit.expectedLateCurrent ? 0 : 1)) {
    throw new Error(`${label}: wrong base current frame count during hover ${audit.visibleCurrentBaseFrameImages}`);
  }
  if (audit.visibleCurrentLateFrameImages !== (audit.expectedLateCurrent ? 1 : 0)) {
    throw new Error(`${label}: wrong late current frame count during hover ${audit.visibleCurrentLateFrameImages}`);
  }
  if (!audit.currentHasNoCompletedBadge
    ...
    throw new Error(`${label}: current stage has conflicting state overlays during hover ${JSON.stringify({
