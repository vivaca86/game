...
export class MainScene extends Phaser.Scene {
  ...
  private renderTurn(notice = ''): void {
    ...
    const recentLogs = this.state.logs.slice(-3).reverse();
    if (recentLogs.length > 0) {
      this.addLine('최근 턴 로그', left, y, '#caf0f8', 16);
      y += 20;
      recentLogs.forEach((log) => {
        this.addLine(`#${log.turn} ${log.phase} ${log.choiceId} / 순이익 ${log.net}`, left, y, '#dee2ff', 14);
        y += 18;
      });
      y += 6;
    }
