...
function appendTurnLog(state: GameState, choiceId: string): GameState {
  const log: TurnLog = {
    turn: state.turn,
    phase: state.phase,
    choiceId,
    net: state.lastSettlement.net,
    summary: state.lastSettlement.summary
  };

  const nextLogs = [...state.logs, log].slice(-8);
  return {
    ...state,
    logs: nextLogs
  };
...
export function resolveTurn(state: GameState, choice: EventChoice): GameState {
  ...
  const loggedState = appendTurnLog(launchedState, choice.id);

  return evaluateEnding(loggedState);
}
