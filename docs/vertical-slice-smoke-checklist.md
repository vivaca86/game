...
1. Continue replacing generated development placeholders one group at a time, keeping `npm.cmd run assets:audit:strict` green. Pipeline/candidate passes now cover card frames, card type icons, all 10 slice card illustrations, attack/disrupt/block intent icons, the two slice backgrounds, two regular monster sprites, the boss sprite, map/rune/relic icons, and the first character portrait. Next likely groups are effect spritesheets, the remaining character spritesheet path, the UI panel asset, or upgrading pipeline placeholders into real visual candidates.
