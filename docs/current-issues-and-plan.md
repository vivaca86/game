...
- Overall active UI goal: about 95% candidate after the WorldMap boss red locked-node split, boss keyboard-selected tooltip audit, open-node pointer tooltip audit, mid sealed-node split, lower-node body/frame pass, locked/future route-state pass, and mobile portrait framing cue audit, following the WorldMap keyboard-selection tooltip audit, WorldMap locked-node tooltip audit, representative keyboard-focus tooltip audit, disabled explanation-tooltip pass, responsive readability-tooltip pass, first visible readability-tooltip pass across ten primary raster scenes, the WorldMap late completed-stack split, route-progress current-leg variant, late current-node body/frame split, mid dormant-node split, and far red locked-node split. This is still WIP rather than final approval, release-ready UI, or user-accepted 95%.
- Static first-view concept matching is ahead of interaction/dynamic-state work.
- WorldMap state truth is still a visible unfinished area, though it now has first-pass runtime current/completed/locked/sealed/dormant overlays, late current-node, mid dormant-node, mid sealed-node, far red locked-node, and boss red locked-node body/frame variants, first concept-derived route-progress bead/thread overlays plus a brighter current-leg route variant, and progressed-save audits for stage 4 current state, stage 6 current / stage 7 mid-sealed, and stage 9 current / stage 10 first-red-lock state.
...
- WorldMap future locked route segments now have first muted `ui_world_map_route_locked_thread_concept` and `ui_world_map_route_locked_bead_concept` overlays. They are source-derived from the same route crop but processed as gray/locked material, rendered only on still-locked forward route segments, and suppressed during keyboard-selection evidence. `tmp/ui-worldmap-action-hit-target-audit.mjs` verifies default, stage-4-progress, and stage-9-progress locked-route counts and placements separately from completed/current cyan route material.
- WorldMap lower route nodes now have a first neutral `ui_world_map_lower_node_body_concept` layer. It is source-derived from the lower completed-node family but processed without the baked number/check badge, rendered on the first five lower map nodes below route overlays and state stacks, and audited in default, stage-4-progress, and stage-9-progress states with `visibleLowerNodeBodies=5`.
...
- WorldMap upper red locked nodes now split the next/first red lock from farther red locks with `ui_locked_stage_far_body_wash_concept` and `ui_locked_stage_far_frame_concept`, and the upper boss-sized red locks with `ui_locked_stage_boss_body_wash_concept` and `ui_locked_stage_boss_frame_concept`. The audit verifies default and stage-4-progress states as 0 next + 4 far + 2 boss locked bodies/frames, and late-progress as 1 next + 3 far + 2 boss locked bodies/frames.
...
- A later 2026-06-14 keyboard-focus tooltip audit added `tools/ui-keyboard-focus-tooltip-audit.mjs`. It proves representative arrow-key focus paths in Town, Dungeon, Combat, Reward, Event, RuneBench, Boss, Result, and Settings show the same DOM readability tooltip layer as pointer hover. The audit verifies focus registry values, tooltip role/aria, title/body length, safe canvas/letterbox placement, pointer-events, z-index, canvas accessibility labels, and screenshots across 1920x1080, 1280x720, and 390x844.
- A later 2026-06-14 WorldMap locked-node tooltip pass added tooltip-only disabled hit targets for locked map nodes without adding an extra default lock-stamp image. Sealed-next, dormant-mid, far red locked, next red locked, and boss red locked stage families now expose danger-tone DOM explanations while remaining non-clickable and leaving the current stage unchanged. `tools/ui-worldmap-locked-tooltip-audit.mjs` verifies all five families across 1920x1080, 1280x720, and 390x844.
- A later 2026-06-14 WorldMap keyboard-selection tooltip pass connects direction-key stage selection to the same DOM readability tooltip layer without turning on an extra hover image. `tools/ui-worldmap-keyboard-tooltip-audit.mjs` verifies lower-left, late-right, and boss-up selection paths across 1920x1080, 1280x720, and 390x844, including one current halo only, no visible Phaser text/vector leak, and safe desktop/mobile placement. Open-stage WorldMap tooltip bodies now include status and room count, so very short stage descriptions are no longer the only readable text.
- A later 2026-06-14 mobile portrait framing pass added `src/ui/overlays/mobileFramingOverlay.ts` and `tools/ui-mobile-framing-audit.mjs`. The DOM cue appears only in 390x844 portrait letterbox space, stays hidden on 1920x1080, 1280x720, and 844x390 landscape, uses `pointer-events: none`, and is immediately suppressed when the readability tooltip appears. The audit covers Town, WorldMap, Dungeon, Combat, Reward, Event, RuneBench, Boss, Result, and Settings and captures evidence under `tmp/ui-quality/mobile-framing/`.
- A later 2026-06-14 WorldMap lower-node body pass added `ui_world_map_lower_node_body_concept`, a source-derived neutral lower-node body layer without the source number/check badge. It renders on the first five lower map nodes below route overlays and state stacks, and `tmp/ui-worldmap-action-hit-target-audit.mjs` verifies five lower-node body images in default, stage-4-progress, and stage-9-progress states.
- A later 2026-06-14 WorldMap lower-node frame pass added `ui_world_map_lower_node_frame_concept`, a number/check-free warm rim layer for the same first five lower map nodes. The extraction mirrors the cleaner left-side source rim to avoid carrying right-side route fragments, renders above the neutral body but below route/state stacks, and the WorldMap audit verifies `visibleLowerNodeFrames=5` with placement/style checks in default, stage-4-progress, and stage-9-progress states.
- A later 2026-06-14 WorldMap mid sealed-node pass added `ui_sealed_stage_mid_body_wash_concept` and `ui_sealed_stage_mid_frame_concept`. The first locked lower nodes keep the base sealed body/frame, while the first locked mid-route sealed node now uses quieter mid sealed material; `tmp/ui-worldmap-action-hit-target-audit.mjs` adds a stage-6/current, stage-7 first-locked evidence case and verifies `visibleSealedMidBodies=1` / `visibleSealedMidFrames=1`.
- A later 2026-06-14 WorldMap boss red locked-node pass added `ui_locked_stage_boss_body_wash_concept` and `ui_locked_stage_boss_frame_concept`. They are extracted from the actual upper boss red-lock crop with number/lock/route/background-safe masking. `WorldMapScene` now uses the boss family for stage indexes 13+ while keeping base red locks for the first non-boss red lock and far red locks for other non-boss red locks; the WorldMap audit now verifies default/stage-4/mid-sealed states as 0 next + 4 far + 2 boss locked bodies/frames and late-progress as 1 next + 3 far + 2 boss locked bodies/frames.
- A follow-up WorldMap boss locked-tooltip pass extended `tools/ui-worldmap-locked-tooltip-audit.mjs` with a `red-boss` case. It seeds stage 13 as the first boss-sized locked node and verifies `stage_dream_arcade` locked explanations across 1920x1080, 1280x720, and 390x844, including unchanged current stage and safe mobile letterbox placement.
- A follow-up WorldMap boss keyboard-selected tooltip pass extended `tools/ui-worldmap-keyboard-tooltip-audit.mjs` with a `boss-up` case. It seeds the upper route around stage 12, presses `ArrowUp`, selects `stage_dream_arcade`, and verifies the choice-tone DOM tooltip across 1920x1080, 1280x720, and 390x844 without adding a second hover image or Phaser text/vector overlay.
- A follow-up WorldMap open-node pointer tooltip pass added `tools/ui-worldmap-open-node-tooltip-audit.mjs`. It verifies lower-open, mid-open, and boss-open unlocked stage nodes across 1920x1080, 1280x720, and 390x844, including choice-tone DOM tooltips, exactly two halo images (current plus hovered target), no Phaser text/vector leak, safe placement, and click selection into the hovered stage.

Next recommended work:

1. Continue refining later current/completed/dormant/locked/sealed-node stage-family variants and remaining baked route/node state against the concept; the late current stack split, late completed stack split, mid dormant split, mid sealed split, far/boss red locked splits, current route-leg split, and locked/future route split are cleaner now but not final node or route recomposition approval.
2. Continue neutralizing or replacing baked node/route geometry where it conflicts with runtime state, especially missing/weak later node variants, lower-node shape silhouettes beyond the first neutral body/frame layer, route-line state marks, and route material that still does not amount to a complete runtime route system.
3. Refine selected/focus/keyboard state art without falling back to Phaser vector overlays. WorldMap directional keyboard selection now also has visible tooltip evidence through lower-left, late-right, and boss-up routes, and Town/Reward/Event/Dungeon/RuneBench/Result keyboard-confirm feedback, Dungeon directional focus, Reward/Event directional choice focus, Town/RuneBench/Result utility focus, Combat/Boss keyboard card/end-turn feedback plus directional focus, Settings keyboard-cancel plus keyboard-focus feedback, and representative focus-triggered DOM tooltip evidence now exist, but this is not final keyboard/focus coverage across screens.
4. Continue deeper pressed/down, disabled, and focus review beyond the audited representative/current controls. Settings' ten audited controls, Town's lower toolbar controls, RuneBench lower confirm, Result lower return, the Town/RuneBench/Result representative utility targets, Combat/Boss cost-disabled cards, and WorldMap locked/sealed/dormant node explanations now have first evidence, but selected/focus state art, broader disabled-state breadth, remaining legacy UX decisions, and final keyboard-focus approval are still missing.
5. Continue dynamic labels/tooltips/accessibility-safe text work beyond the first hidden DOM label pass, first visible tooltip pass, responsive tooltip placement audit, representative disabled explanation pass, representative keyboard-focus tooltip audit, WorldMap locked-node tooltip audit, WorldMap keyboard-selection tooltip audit, and WorldMap open-node pointer tooltip audit. Representative safe tooltip zones now exist, including mobile portrait letterbox placement, locked-control explanations, focus-triggered tooltips, WorldMap locked-node explanations, WorldMap selected-stage explanations, and lower/mid/boss open-node pointer explanations, but broader gameplay-critical readable text, full selected/focus approval, and user acceptance are still unfinished.
6. Continue mobile/responsive review beyond the first portrait framing cue. The game now has a deliberate mobile orientation/framing cue in unused portrait letterbox space, but final mobile UX approval, device/browser breadth, and user acceptance are still missing.
7. Continue WorldMap recomposition beyond neutralized samples, first-pass badges, the first lower-node neutral body/frame layer, the first current/completed/locked/sealed/dormant body/frame overlays, the late current/completed, mid sealed, mid dormant, and far/boss red locked splits, and the first completed/current/locked route bead/thread overlays. The latest stage-5/stage-4 corrections and state body/frame/route/lower-node passes reduce stale state reads but do not replace full current/completed/locked/sealed/dormant body variants or a complete dynamic route-state system.

Known unfinished scope:

- Full WorldMap current/completed/locked/sealed/dormant recomposition beyond first-pass badges, the first lower-node neutral body/frame layer, the late current/completed-stack, mid sealed, mid dormant, and far/boss red locked splits, body washes, frames, and completed/current/locked route bead/thread overlays.
- Selected/focus/keyboard state art beyond the first WorldMap directional-selection plus selected-stage tooltip pass, now including lower-left, late-right, and boss-up WorldMap routes, Town/Reward/Event/Dungeon/RuneBench/Result keyboard-confirm feedback pass, Dungeon directional focus pass, Reward/Event directional choice-focus pass, Town/RuneBench/Result utility-focus pass, Combat/Boss keyboard-action plus directional focus pass, Settings keyboard-cancel/focus feedback pass, and representative focus-triggered tooltip audit.
- Broad disabled-state coverage beyond Event unaffordable choice, Combat/Boss cost-disabled cards, and WorldMap locked/sealed/dormant node explanations.
- Screen-specific pressed/down art beyond the currently audited controls.
- Dynamic readability and visible tooltip/safe-text zones beyond the first hidden accessibility-label layer, representative visible tooltip pass, responsive tooltip placement audit, first disabled explanation pass, representative keyboard-focus tooltip audit, WorldMap locked-node tooltip audit, WorldMap keyboard-selection tooltip audit including the boss-up path, and WorldMap open-node pointer tooltip audit.
- Mobile portrait UX/framing beyond the first automated responsive sanity audit and the first portrait framing cue. The 390x844 pass proves no clipping and now shows a non-blocking orientation cue in letterbox space, but final mobile UX approval still needs review.
