...
- A later 2026-06-14 WorldMap locked-node tooltip pass added tooltip-only disabled hit targets for locked map nodes without adding an extra default lock-stamp image. Sealed-next, dormant-mid, far red locked, and next red locked stage families now expose danger-tone DOM explanations while remaining non-clickable and leaving the current stage unchanged. `tools/ui-worldmap-locked-tooltip-audit.mjs` verifies all four families across 1920x1080, 1280x720, and 390x844.
