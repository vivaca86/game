...
This checkpoint adds route-family evidence for pointer-down/pressed state on unlocked WorldMap stage nodes. The new audit covers lower-open, mid-open, and boss-open nodes across 1920x1080, 1280x720, and 390x844. Each case verifies the expected pressed-size halo dimensions and alpha, retained choice-tone tooltip content, the original current marker/body/frame/status stack staying anchored to the current node, correct base/late current material choice, no conflicting current-node state overlays, no visible Phaser text/vector leak, safe desktop/mobile placement, and pointer-up selection into the pressed stage.
