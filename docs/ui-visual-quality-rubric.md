...
- The current textless raster-only checkpoints now have a first hidden dynamic accessibility-label pass, a first automated responsive sanity pass, and a representative visible readability-tooltip pass across ten primary raster scenes with 1920/1280/mobile portrait evidence. The mobile portrait tooltip now uses letterbox space instead of covering the canvas. The screens still rely heavily on baked concept information and need broader gameplay-critical readable text review, disabled explanation coverage, full mobile framing/orientation review, user acceptance, and final concept-match approval.
