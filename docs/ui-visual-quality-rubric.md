...
- The same WorldMap path now has first source-derived `ui_world_map_route_progress_bead_concept` and `ui_world_map_route_progress_thread_concept` overlays on progressed route segments, plus brighter `ui_world_map_route_progress_current_bead_concept` and `ui_world_map_route_progress_current_thread_concept` variants for the final/current leg. They are verified in default, stage-4-progress, stage-9-progress, and keyboard-selected states, but they are still conservative route-progress layers rather than a full dynamic route recomposition system.
