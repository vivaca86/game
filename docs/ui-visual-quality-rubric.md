...
- The current textless raster-only checkpoints now have a first hidden dynamic accessibility-label pass, a first automated responsive sanity pass, a representative visible readability-tooltip pass across ten primary raster scenes with 1920/1280/mobile portrait evidence, first disabled explanation tooltips for Event plus Combat/Boss cost-disabled cards, representative keyboard-focus tooltip evidence across nine focus paths, WorldMap locked-node explanation tooltips across sealed/dormant/red locked families, and WorldMap keyboard-selected stage tooltip evidence. The mobile portrait tooltip now uses letterbox space instead of covering the canvas. The screens still rely heavily on baked concept information and need broader gameplay-critical readable text review, broader disabled-state breadth, full selected/focus approval beyond audited representatives, full mobile framing/orientation review, user acceptance, and final concept-match approval.
