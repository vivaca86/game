...
### Problem: WorldMap future route state still relied on the baked underlay

- Cause: Completed and current route legs had source-derived cyan thread/bead overlays, but forward locked route segments still mostly depended on the neutralized baked concept underlay. That meant the route system had completed/current evidence without a separate locked/future material family.
- Impact: The map could show current and completed travel progress while still leaving future route state visually ambiguous. This weakened the route recomposition story because node state had current/completed/locked/sealed/dormant families, but route state only had completed/current overlays.
- Resolution: Added `ui_world_map_route_locked_thread_concept` and `ui_world_map_route_locked_bead_concept`, both extracted from the WorldMap route source crop with muted gray locked processing. `WorldMapScene` renders them on forward route segments whose destination stage is still locked, and suppresses them during keyboard-selection evidence. `tmp/ui-worldmap-action-hit-target-audit.mjs` now verifies locked route counts and placements separately from base/current route material.
- Prevention: Route-state audits should track each route texture family separately: completed/base, current/final, and locked/future. Do not treat underlay visibility as proof that runtime route state is recomposed.

### Problem: WorldMap lower-node silhouettes only had neutralized underlay evidence

- Cause: The lower 1-5 WorldMap nodes had many runtime state overlays, and the strongest baked check/current scars were neutralized in the underlay, but there was no separate neutral runtime body layer for the lower-node family itself.
- Impact: The lower nodes could still read as partly baked concept geometry underneath current/completed/sealed/dormant overlays. This made the lower-node recomposition story weaker than the later state-family splits and the completed/current/locked route material splits.
- Resolution: Added `ui_world_map_lower_node_body_concept`, extracted from the lower completed-node source crop but flattened so the source number/check badge does not carry into runtime states. `WorldMapScene` renders it on the first five lower nodes below route overlays and state stacks. `tmp/ui-worldmap-action-hit-target-audit.mjs` verifies five lower-node body images with placement, size, and alpha checks in default, stage-4-progress, and stage-9-progress states.
- Prevention: Lower-node recomposition should have runtime asset evidence, not only neutralized underlay samples. Future passes should continue toward state-specific body variants, but common neutral layers must stay number/check-free and be audited separately from state badges.

### Problem: WorldMap lower-node neutral body still looked too flat without a matching rim

- Cause: The first lower-node neutral body layer reduced baked silhouettes, but the lower 1-5 nodes still lacked a separate runtime frame/rim layer. Reusing a full source node frame directly risked carrying baked numbers, completed checks, and route fragments.
- Impact: The lower-node recomposition pass improved material coverage but could still read flatter than the current/completed/sealed/dormant state stacks above it. It also left the lower-node family with body evidence but no matching frame evidence in the WorldMap audit.
- Resolution: Added `ui_world_map_lower_node_frame_concept`, extracted from the lower completed-node source crop as a number/check-free warm rim. The extraction mirrors the cleaner left-side rim to avoid right-side route fragments. `WorldMapScene` renders it above the neutral lower-node body and below route/state stacks. `tmp/ui-worldmap-action-hit-target-audit.mjs` verifies five lower-node frame images with placement, size, and alpha checks in default, stage-4-progress, and stage-9-progress states.
- Prevention: Lower-node audits should check body and frame layers separately. Common reusable lower-node material must remain number/check-free and route-fragment-safe; stronger state-specific variants can build on top of it later.

### Problem: Mid-route sealed WorldMap nodes reused the lower sealed body and frame stack

- Cause: The sealed state had one body/frame texture family for the first lower/mid locked node. Dormant nodes already had lower/mid variants, but sealed nodes did not, so a first locked mid-route node reused the stronger lower sealed material.
- Impact: The WorldMap state-family split was uneven. The audit could prove that a sealed body/frame existed, but it could not prove that mid-route sealed nodes had their own quieter material language.
- Resolution: Added `ui_sealed_stage_mid_body_wash_concept` and `ui_sealed_stage_mid_frame_concept`, extracted from the gray sealed source crop with quieter `sealedMid` processing. `WorldMapScene` keeps the base sealed stack for lower first-locked nodes and uses the mid sealed stack for first-locked nodes after the lower 1-5 family. `tmp/ui-worldmap-action-hit-target-audit.mjs` now includes a stage-6/current, stage-7 first-locked evidence case and verifies base/mid sealed body/frame counts separately.
- Prevention: Sealed-state audits should track lower/base and mid-route texture-family choice, not just total sealed count. When a state family already changes placement and alpha by route band, the transferable body/frame source should be split and audited too.

### Problem: Upper boss red locks still reused the non-boss far locked material

- Cause: The previous red locked-node split separated the first/next red lock from farther red locks, but stage indexes 13+ are larger boss-shaped red locks and still used the same `ui_locked_stage_far_body_wash_concept` / `ui_locked_stage_far_frame_concept` family as ordinary far red locks.
...
- Impact: The UI could claim open-node pointer support while still lacking strong evidence that the clicked node becomes the visual current node with the correct base/late current material and without stale hover/tooltip artifacts. That left a selected-state gap in the 95% candidate claim.
- Resolution: Added `tools/ui-worldmap-open-node-selection-audit.mjs`. It clicks lower-open, mid-open, and boss-open unlocked nodes across 1920x1080, 1280x720, and 390x844, waits for `context.run.stageId` to match the clicked target, moves the pointer away, and verifies the current marker, halo, body, frame, and status badge are anchored to the selected node. It also verifies base current material on the lower target, late current material on mid/boss targets, no conflicting state overlays on the selected node, no Phaser text/vector leak, no stale tooltip, and a `flow:stage_select:<stageId>` log entry.
- Prevention: Treat click selection, pressed feedback, and hover readability as separate evidence. Whenever a WorldMap node family has a pointer path, audit the post-click selected/current visual stack in addition to the hover/down moments.

### Problem: Locked WorldMap hover/click did not prove the original current stack stayed stable

- Cause: The locked-node tooltip audit proved danger-tone tooltip content, non-clicking behavior, and unchanged current stage id, but it did not verify the full current marker/body/frame/status stack while the pointer hovered or clicked a locked target.
- Impact: Locked-node evidence could pass while the current node lost a layer, used the wrong base/late material family, or blended with stale completed/locked/sealed/dormant overlays. That left disabled/readability proof weaker than the open-node hover/down current-stack audits.
- Resolution: Strengthened `tools/ui-worldmap-locked-tooltip-audit.mjs`. It now verifies sealed-next, dormant-mid, red-far, red-next, and red-boss locked node hover/click paths across 1920x1080, 1280x720, and 390x844 with unchanged `context.run.stageId`, retained current marker, halo, body, frame, and status badge, correct base/late current material choice, no conflicting current-node overlays, no Phaser text/vector leak, and continued danger-tone tooltip evidence.
- Prevention: Disabled-node audits should verify both the blocked target behavior and the persistent current-state stack. A locked tooltip plus unchanged stage id is not enough when the WorldMap visual language depends on layered current marker/body/frame/status assets.

### Problem: Locked WorldMap target nodes did not prove their own state stack during hover/click

- Cause: The strengthened locked-node audit verified the original current stack during locked interactions, but target state-family proof still came mostly from the broader WorldMap action audit. The locked interaction path did not directly check that the hovered/clicked target kept its own sealed, dormant, far red, next red, or boss red body/frame/badge stack.
- Impact: Disabled-state evidence could still over-rely on unchanged current state and tooltip copy while missing a regression where the locked target lost its body/frame, used the wrong locked family, or blended with current/completed overlays during hover/click.
- Resolution: Strengthened `tools/ui-worldmap-locked-tooltip-audit.mjs` again. The audit now verifies target body/frame/badge placement and style for sealed-next, dormant-mid, red-far, red-next, and red-boss families across 1920x1080, 1280x720, and 390x844, and checks that no current, completed, or wrong locked/sealed/dormant stack appears on the target during locked hover/click.
- Prevention: Disabled-state audits should pair current-state stability with target-state family proof. For WorldMap locked nodes, do not count tooltip behavior as enough unless the blocked target's own bitmap stack is also verified.

### Problem: Open WorldMap hover/down targets did not prove their own completed stack

- Cause: The strengthened open-node hover/down audits verified the original current stack stayed stable while another open node was hovered or pressed, but the target node itself was still mainly checked for hover/pressed halo feedback. The audits did not directly verify that the hovered/pressed target kept the correct completed base or late body/frame/badge family.
- Impact: Open-node evidence could pass while leaving a gap where the target completed stack was missing, using the wrong base/late completed family, or accidentally receiving current/locked overlays during hover/down. That made open-node target evidence weaker than the locked-node target-stack audit.
- Resolution: Strengthened `tools/ui-worldmap-open-node-tooltip-audit.mjs` and `tools/ui-worldmap-open-node-down-audit.mjs` again. Both audits now verify target completed body/frame/badge placement and style for lower-open `completed-base` and mid/boss `completed-late` targets across 1920x1080, 1280x720, and 390x844, while also checking no current marker/body/frame/status stack and no locked/sealed/dormant stack appears on the target during hover/down.
- Prevention: Pointer hover/down audits should pair source/current-state stability with target-state family proof. For WorldMap open nodes, do not count a hover or pressed halo as enough unless the target node's persistent completed bitmap stack is also verified.

### Problem: WorldMap route stacks were not audited during node interactions

- Cause: The WorldMap action audit verified completed/current/locked route thread and bead counts in static map states, while node interaction audits focused on tooltips, node body/frame/badge stacks, and current-stage stability. That left route overlays less directly proven during open-node hover/down and locked-node hover/click.
- Impact: A regression could preserve node stacks and tooltips while dropping route threads/beads, using the wrong base/current/locked route family, losing rotation/placement, or leaking the old `ui_hover_route_node_concept` route image during interaction.
- Resolution: Added `tools/ui-worldmap-route-interaction-audit.mjs`. It covers open lower/mid/boss hover and down states plus locked sealed/dormant/red/boss hover and click states across 1920x1080, 1280x720, and 390x844. It verifies base completed route threads/beads, current/final route threads/beads, and muted locked/future route threads/beads by count, placement, display size, alpha, and rotation, and verifies the old route-hover image count stays zero.
- Prevention: Route-state audits should be paired with interaction audits when the interaction changes hover/down/disabled state around the same map. Static route-state correctness is not enough evidence that route overlays remain stable during pointer feedback.

### Problem: Open WorldMap click selection did not prove route-stack recomposition

- Cause: `tools/ui-worldmap-open-node-selection-audit.mjs` proved that clicking lower/mid/boss open nodes changed `context.run.stageId` and moved the selected/current marker/body/frame/status stack, but it did not inspect the route overlays after the selected current stage changed.
- Impact: The project could prove route stability during hover/down and locked interactions while still leaving a gap where post-click selection might leave stale completed/current/locked route thread or bead stacks from the previous current stage. That weakened the 95% candidate claim for route recomposition.
- Resolution: Strengthened `tools/ui-worldmap-open-node-selection-audit.mjs` to verify post-click route-stack counts, placement, display size, alpha, and rotation for base completed route threads/beads, current/final route threads/beads, and muted locked/future route threads/beads. The audit also verifies the old `ui_hover_route_node_concept` route-hover image count stays zero after selection.
- Prevention: Selection audits should verify both the selected/current node stack and route-stack recomposition whenever the selected stage changes. Interaction stability evidence before click is not enough when the route system is derived from the current stage.

### Problem: WorldMap keyboard-selected state only suppressed routes in one older audit case

- Cause: Runtime intentionally suppresses WorldMap route overlays while a keyboard-selected tooltip is pending, but the current keyboard tooltip audit only verified selected/current stack and tooltip behavior. The older action audit checked route suppression for a single lower-left 1920 case, not the lower/mid/boss keyboard-selected families across responsive viewports.
