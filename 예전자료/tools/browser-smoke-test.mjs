...
await page.waitForSelector(".action-feedback-card .feedback-pulse-rail", { timeout: 10000 });
await page.waitForSelector(".action-feedback-card .feedback-pulse-step", { timeout: 10000 });
...
await page.waitForSelector(".combat-impact-path", { timeout: 10000 });
await page.waitForSelector(".combat-impact-node", { timeout: 10000 });
...
if (!actionFeedbackText.includes("카드") || !actionFeedbackText.includes("대상") || !actionFeedbackText.includes("결과")) {
  throw new Error("카드 사용 피드백 흐름 표시 실패");
}
...
if (!combatImpactText.includes("카드") || !combatImpactText.includes("대상") || !combatImpactText.includes("결과")) {
  throw new Error("전투 결과 흐름 표시 실패");
}
...
const actionFeedbackOverflowItems = await page.locator(".action-feedback, .action-feedback-copy, .feedback-stat, .feedback-pulse-step, .combat-impact-strip, .combat-impact-chip, .combat-impact-node, .enemy-impact-chip, .enemy-hit").evaluateAll((elements) => elements
  .filter((element) => element.scrollWidth > element.clientWidth + 2 || element.scrollHeight > element.clientHeight + 2)
  .map((element) => element.className));
...
await page.locator(".feedback-pulse-rail").screenshot({ path: "tmp/action-feedback-pulse-desktop.png" });
await page.locator(".combat-impact-path").screenshot({ path: "tmp/combat-impact-path-desktop.png" });
await page.screenshot({ path: "tmp/action-feedback-desktop.png", fullPage: true });
await page.setViewportSize({ width: 390, height: 820 });
await page.waitForTimeout(120);
const actionFeedbackMobileOverflow = await page.locator(".action-feedback, .action-feedback-copy, .feedback-stat, .feedback-pulse-step, .combat-impact-strip, .combat-impact-chip, .combat-impact-node, .enemy-impact-chip").evaluateAll((elements) => elements
  .filter((element) => element.scrollWidth > element.clientWidth + 2 || element.scrollHeight > element.clientHeight + 2)
  .map((element) => element.className));
...
await page.locator(".feedback-pulse-rail").screenshot({ path: "tmp/action-feedback-pulse-mobile.png" });
await page.locator(".combat-impact-path").screenshot({ path: "tmp/combat-impact-path-mobile.png" });
