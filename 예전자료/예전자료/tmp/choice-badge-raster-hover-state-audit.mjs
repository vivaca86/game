...
async function startServer() {
  for (const port of [4193, 4194, 4195, 4196]) {
    try {
...
async function readDebugMap(page) {
...
const cases = [
  {
    ...
    hoverX: 630,
    hoverY: 618,
    expectedPhaseAfterClick: "event",
    screenshot: "tmp/ui-quality/reward-raster-choice-badge-hover-state-v1-1920.png"
  },
  {
    label: "event-choice",
    ...
    hoverX: 618,
    hoverY: 722,
    expectedPhaseAfterClick: "rune_bench",
    screenshot: "tmp/ui-quality/event-raster-choice-badge-hover-state-v1-1920.png"
  }
];
...
try {
  ...
  for (const testCase of cases) {
    ...
    const stats = await readSceneStats(page, testCase.sceneName, testCase.underlayKey, "ui_hover_choice_badge_concept");
    ...
    await page.mouse.click(box.x + (testCase.hoverX / 1920) * box.width, box.y + (testCase.hoverY / 1080) * box.height);
    await page.waitForFunction((expectedPhase) => {
      const overlay = document.querySelector("#debug-overlay");
      return overlay?.textContent?.includes(`phase=${expectedPhase}`);
    }, testCase.expectedPhaseAfterClick, { timeout: 10000 });

    results.push({
      ...testCase,
      screenshot: path.resolve(testCase.screenshot),
      delta,
      phaseAfterClick: (await readDebugMap(page)).phase,
      ...stats
    });
    await page.close();
  }
